...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	*/
	...
	else
	{
		...
		pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_CENTER);
		//pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_RIGHT);
	} // if
} // End of DrawTheText
...
DWORD CButtonST::SetImage(LPCTSTR lpImagePath,COLORREF crTransColor){
	HBITMAP bmp = CEnBitmap::LoadImageFile(lpImagePath);
	if(bmp==NULL)return 0;
	return SetBitmaps(bmp,crTransColor); 
}
